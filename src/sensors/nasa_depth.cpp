...
{
  debugI("Notified of %d bytes on I2C", numBytes);
  if (numBytes !=11) {
...
//className = "NasaDepth";
  ...
  debugI("Added I2C onReceive handler");
}

void NasaDepth::enable() {
  app.onRepeat(read_delay,[this]() {
    if (new_data) {
      debugI("New data on Depth");
      output = DecodeData();
      if (output >= 0) {
        this->notify();
      }
...
void NasaDepth::get_configuration(JsonObject& root) {
...
// The NASA Clipper sends a 12 byte I2C data packet, this data is directly sent to a pcf8566p
